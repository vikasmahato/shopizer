...
<script type="text/javascript">
	...
	$(function(){		
		...
		<c:forEach items="${vendor.descriptions}" var="description" varStatus="counter">
	...
	function removeImage(imageId){
			...
			$.ajax({
			  type: 'POST',
			  url: '<c:url value="/admin/vendors/vendor/removeImage.html"/>',
	...
	function validateCode() {
		...
		var code = $("#code").val();
		var id = $("#id").val();
		checkCode(code,id,'<c:url value="/admin/vendor/checkCode.html" />');
	}
...
<div class="tabbable">
		 ...
		 <div class="tab-content">
			...
			<div class="tab-pane active" id="catalogue-section">
				...
				<h3>
					<c:choose>
						<c:when test="${vendor.vendor.id!=null && vendor.vendor.id>0}">
							<s:message code="label.vendor.edit" text="Edit vendor" /> <c:out value="${product.product.sku}"/>
						</c:when>
						<c:otherwise>
							<s:message code="label.vendor.create" text="Create a vendor" />
						</c:otherwise>
					</c:choose>
				</h3>
				<br/><br/>
								
				<c:url var="vendorSave" value="/admin/catalogue/vendor/save.html"/>
 
				<form:form method="POST" modelAttribute="vendor" action="${vendorSave}">
					...
					<div class="control-group">
						<label><s:message code="label.entity.phone" text="Phone"/></label>
						<div class="controls">
							<form:input cssClass="input-large highlight" path="phone"/>
							<span class="help-inline"><div id="checkPhoneStatus" style="display:none;"></div><form:errors path="phone" cssClass="error" /></span>
						</div>
					</div>

					<div class="control-group">
						<label><s:message code="label.entity.name" text="Phone"/></label>
						<div class="controls">
							<form:input cssClass="input-large highlight" path="name"/>
							<span class="help-inline"><div id="checkNameStatus" style="display:none;"></div><form:errors path="name" cssClass="error" /></span>
						</div>
					</div>

					<div class="control-group">
						<label><s:message code="label.entity.email" text="Email"/></label>
						<div class="controls">
							<form:input cssClass="input-large highlight" path="email"/>
							<span class="help-inline"><div id="checkEmailStatus" style="display:none;"></div><form:errors path="email" cssClass="error" /></span>
						</div>
					</div>
					...
					<c:forEach items="${vendor.descriptions}" var="description" varStatus="counter">
						...
						<div class="control-group">
							<label class="required"><s:message code="label.vendoredit.vendorname" text="Vendor Name"/> (<c:out value="${description.language.code}"/>)</label>
							<div class="controls">
						...
						<div class="control-group">
							<label><s:message code="label.vendoredit.vendortitle" text="vendor Title"/> (<c:out value="${description.language.code}"/>)</label>
							<div class="controls">
						...
						<div class="control-group">
							<label><s:message code="label.vendoredit.vendorurl" text="URL"/> (<c:out value="${description.language.code}"/>)</label>
							<div class="controls">
						...
						<div class="control-group">
							<label class="required"><s:message code="label.vendoredit.vendordescription" text="vendor Description"/> (<c:out value="${description.language.code}"/>)</label>
							<div class="controls">

	                        			 <textarea cols="30" id="descriptions${counter.index}.description" name="descriptions[${counter.index}].description">
	                        				<c:out value="${vendor.descriptions[counter.index].description}"/>
	                        			 </textarea>
							</div>
							<script type="text/javascript">
								//<![CDATA[
                 	...
                 	<div class="control-group">
                        <label><s:message code="label.vendoredit.vendororder" text="Order"/></label>
                        <div class="controls">
