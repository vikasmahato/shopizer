 ...
 */
@Component
public class BrandShippingCodeOrderTotalModuleImpl implements OrderTotalPostProcessorModule {
	
	private static final Logger LOGGER = LoggerFactory.getLogger(BrandShippingCodeOrderTotalModuleImpl.class);
	...
	//@Inject
	//KieContainer kiebrandBasedPricingContainer;
	...
	public OrderTotal caculateProductPiceVariation(final OrderSummary summary, ShoppingCartItem shoppingCartItem, Product product, Customer customer, MerchantStore store)
		...
		Validate.notNull(product.getbrand(),"product brand must not be null");
		...
		inputParameters.setItembrandCode(product.getbrand().getCode());
...
/*        KieSession kieSession = kiebrandBasedPricingContainer.newKieSession();
